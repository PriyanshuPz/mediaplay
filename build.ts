...
async function copyAssets(outdir: string) {
  const assetsSrc = path.join(process.cwd(), "fe/assets");
  const assetsDest = path.join(outdir, "assets");

  if (!existsSync(assetsSrc)) {
    console.log("ℹ️ No assets folder found, skipping copy");
    return;
  }
  ...
  console.log(`📦 Assets copied → ${path.relative(process.cwd(), assetsDest)}`);
}
...
const entrypoints = [...new Bun.Glob("**.html").scanSync("fe")]
  .map((a) => path.resolve("fe", a))
